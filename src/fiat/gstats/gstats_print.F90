...
IF (LTRACE_STATS) THEN
  ...
  DO JCALL=1,MIN(NCALLS_TOTAL,NTRACE_STATS)
    ...
    ENDIF
    IF (IACTION == 1) THEN
      WRITE(KULOUT,'(1X,F10.3,1X,I4,1X,A,1X,A,1X,A,1X,F10.3)') &
       &TIME_TRACE(JCALL),ICALLER,CCTYPE(ICALLER),CCDESC(ICALLER),CLACTION(IACTION),&
       &ZTHISTIME(ICALLER)+(TIME_TRACE(JCALL)-ZTIMELCALL(ICALLER))
    ELSE
      WRITE(KULOUT,'(1X,F10.3,1X,I4,1X,A,1X,A,1X,A)') TIME_TRACE(JCALL),ICALLER,CCTYPE(ICALLER),CCDESC(ICALLER),&
       & CLACTION(IACTION)
    ENDIF
  ENDDO
ENDIF
